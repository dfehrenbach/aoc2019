...
(def lines
  (map #(str/split % #",")
       (str/split (slurp "resources/day3/input.txt") #"\r\n")))
...
(defn part1-solution [lines]
  (let [[graph1-points graph2-points] (map (comp set build-lines create-points) lines)
        ...
        data                          (map (fn [cross distance] {:cross    cross
                                                                 :distance distance}) crosses distances)]
...
(defn part2-solution [lines]
  (let [[graph1-points graph2-points]            (map (comp build-lines create-points) lines)
        ...
        {wire1-distance :l1
         wire2-distance :l2} (apply min-key #(+ (:l1 %) (:l2 %)) distances)]
